...
2. Commit metadata transaction (stats, map table, intervals)
3. Update `numerics`, bump `signal`, and advance checkpoint positions
...
- Allocation uses chunked memory for small payloads and heap for large payloads
...
- **TxnKV** (read-write):
  - records `Begin` and WAL updates on first mutation
  - commits by `record_commit` + `stabilize` (flush + checkpoint)
...
- `Logging::checkpoint` is triggered by `numerics.signal` changes
- Checkpoint ensures buffered WAL data is flushed before a `WalCheckpoint` is written
- `last_ckpt` is updated based on flush results and active txns
...
- Phase 1: load WAL descriptors and build `Context`
